    ...
    public static function checkoutToGateway($data) // getting all parameter in one array
    {
        ...
        if (!in_array($payment_gateway, ['bank_transfer', 'manual_payment_']) && $checkout_type === 'digital') {
            ...
            if (!method_exists((new PaymentGatewayCredential()), $credential_function)) {
                ...
                $custom_data['cancel_url'] = route(PaymentRouteEnum::CANCEL_ROUTE, $data['order_log_id']);
                ...
                $custom_data['success_url'] = route(PaymentRouteEnum::SUCCESS_ROUTE, $custom_data['payment_details']['id']);
                ...
                if (class_exists($charge_customer_class_namespace) && method_exists($custom_charge_customer_class_object, $charge_customer_method_name)) {
                    ...
                    $success_url = $custom_data['success_url'];
                    $cancel_url = $custom_data['cancel_url'];

                    $dataToStore = [
                        'bkash_payment_details' => ["payment_for" => $custom_data['payment_for'], "payment_details" => $custom_data['payment_details']],
                        'bkash' => [
                            'invoice_amount' => $custom_data['total'],
                    ...
                    session()->put($dataToStore);
                    $payment = PaymentGateway::where('name', 'bkash')->first();
                    new BkashController($payment->credentials);
                    return to_route('tenant.pay-with-bkash');
                ...
                try {
                    session()->put('bkash_payment_details',['payment_for'=>'tenant','order_id' => $payment_details->id]);
