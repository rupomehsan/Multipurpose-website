...
class MediaUploaderController extends Controller
{
    ...
    public function upload_media_file(Request $request)
    {
        ...
        if ($request->hasFile('file')) {
            ...
            if (in_array($image_extenstion, ['pdf', 'doc', 'docx', 'txt', 'svg', 'zip', 'csv', 'xlsx', 'xlsm', 'xlsb', 'xltx', 'pptx', 'pptm', 'ppt', 'avif'])) {
                ...
                Storage::disk('s3')->putFileAs($folder_path,$request->file('file'),$image_db ,'public');
    ...
    private function handle_image_upload(
        ...
        if ($image_width > 150) {
            ...
            // store $resize_thumb_image to s3
            $binaryData = $resize_thumb_image->stream()->__toString();
            $store_path = $folder_path . '/thumb/' . $image_thumb;
            Storage::disk('s3')->put($store_path, $binaryData, 'public');

            // store $resize_grid_image to s3
            $binaryData = $resize_grid_image->stream()->__toString();
            $store_path = $folder_path . '/grid/' . $image_grid;
            Storage::disk('s3')->put($store_path, $binaryData, 'public');

            // store $large image
            $binaryData = $resize_large_image->stream()->__toString();
            $store_path = $folder_path . '/large/' . $image_large;
            Storage::disk('s3')->put($store_path, $binaryData, 'public');
