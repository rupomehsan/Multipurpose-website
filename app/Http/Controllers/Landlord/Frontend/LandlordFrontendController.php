...
class LandlordFrontendController extends Controller
{
    ...
    protected function tenant_user_create(Request $request)
    {
        ...
        // dd($this->generateReferCode());

        $request->validate([
            'first_name' => ['required', 'string', 'max:95'],
            'last_name' => ['required', 'string', 'max:95'],
        ...
        if ($request->input('refer_code')) {
            $referUser = User::where('referral_code', $request->input('refer_code'))->first();
            $referUserId = $referUser->id;
        }
        ...
        $user_id = User::create([
            ...
            'referral_code' => $this->generateReferCode(),
            'refer_user_id' => $referUserId,
        ])->id;

        if ($referUserId) {
            $bonus = 100;
            $existWallet = Wallet::where('user_id', $referUserId)->first();
            if ($existWallet) {
                $existWallet->balance += $bonus;
                $existWallet->update();
            } else {
                $newWallet = new Wallet();
                $newWallet->user_id = $referUserId;
                $newWallet->balance = $bonus;
                $newWallet->save();
            }
        }
        ...
        // return redirect()->route('landlord.user.home');
        return redirect()->route('landlord.user.business.create');
    }
    ...
    public function generateReferCode()
    {
        ...
        while (!$unique && $attempts < $maxAttempts) {
            ...
            if (!User::where('referral_code', $referenceCode)->exists()) {
                $unique = true;
            }
    ...
    public function tenant_create_user_business()
    {
        if (auth('web')->check()) {
            return redirect()->route('tenant.user.home');
        }
        $themes = Themes::paginate(12);
        return view('landlord.frontend.user.create-business', compact('themes'));
    }
    public function getAllTheme()
    {
        $themes = Themes::paginate(12);

        $view = view('landlord.frontend.user.theme-list', compact('themes'));
        return $view;
    }
}
