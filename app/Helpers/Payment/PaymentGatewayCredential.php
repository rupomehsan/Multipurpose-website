...
class PaymentGatewayCredential
{
    ...
    public static function get_bkash_credential()
    {
        $bkash_credential_from_database = PaymentGateway::where('name', 'bkash')->first();

        $decoded = json_decode($bkash_credential_from_database->credentials);

        $mode = $bkash_credential_from_database->test_mode;
        $bkash_app_key = $mode == 1 ? $decoded->bkash_app_key : $decoded->bkash_app_key;
        $bkash_app_secret = $mode == 1 ? $decoded->bkash_app_secret : $decoded->bkash_app_secret;
        $bkash_username = $mode == 1 ? $decoded->bkash_username : $decoded->bkash_username;
        $bkash_password = $mode == 1 ? $decoded->bkash_password : $decoded->bkash_password;
        // $test_mode = $bkash_credential_from_database->test_mode == 1;

        $bkash = new BkashTokenizePaymentController();
        
        $bkash->setAppKey($bkash_app_key);
        $bkash->setAppSecret($bkash_app_secret);
        $bkash->setAppUsername($bkash_username);
        $bkash->setAppPassword($bkash_password);
        
        session()->put('bkash_app_key',$bkash_app_key);
        session()->put('bkash_app_secret',$bkash_app_secret);
        session()->put('bkash_username',$bkash_username);
        session()->put('bkash_password',$bkash_password);
        session()->put('sandbox',$mode == 1? true : false);

        config()->set("bkash.bkash_app_key", $bkash_app_key);
        config()->set("bkash.bkash_app_secret", $bkash_app_secret);
        config()->set("bkash.bkash_username", $bkash_username);
        config()->set("bkash.bkash_password", $bkash_password);
        config()->set('bkash.sandbox', $mode == 1? true : false);
        return $bkash;
    }
